...
static int bugfix __read_mostly = 0;
...
module_param(bugfix, int, 0644);
MODULE_PARM_DESC(bugfix, "Bug fix: 0=no; 1=b1; 2=b2; 4=b3; 3=b1,b2; 5=b1,b3; 6=b2,b3; 7=b1,b2,b3");
...
if (bugfix&1){
	...
	printk("epoch_start=%u, last_max_cwnd=%u, cwnd=%u, bic_K=%u, bic_origin_point=%u, delay_min=%u, bic_target=%u, delta=%u, cnt=%u, t=%lu, offs=%lu\n", 
...
if (bugfix &2){
	//printk("BUGFIX 2\n");
    if (cwnd < ca->bic_origin_point) // Lisong; to be consistent with the latest RFC
        delta = (cwnd * scale) >> 3;
    else
		delta = cwnd;
}
...
if(bugfix&4){
	//printk("BUGFIX 3\n");
    // Lisong: should use the expected cwnd in the next RTT instead of current cwnd
