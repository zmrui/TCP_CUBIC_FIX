...
static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
{
	...
	if(ntohs(th->source)==5202){
		if (strcmp(icsk->icsk_ca_ops->name,"cubic")==0){
			struct bictcp {
			...
			printk("ackseq:%u ca->epoch_start:%u ca->cnt:%u ca->ack_cnt:%u ca->tcp_cwnd:%u tp->snd_cwnd:%u",
					ntohl(th->ack_seq),ca->epoch_start, ca->cnt, ca->ack_cnt, ca->tcp_cwnd, tp->snd_cwnd);
		}
	}
	return 1;
